...
class DataProcessor:
    ...
    def _apply_normalization(
        ...
        if v_mode == "phys_minmax":
            ...
            for ch in (idx_vch, idx_vdis):
                ...
                features[:, ch, :] = X
            norm_stats["voltage"] = {"mode": "phys_minmax", "v_min": vmin, "v_max": vmax}
        else:
        ...
        for family in ("dvdq", "d2vdq2", "dqdv"):
    ...
    def create_capacity_features_and_masks(
        ...
        for k, ((ds, cyc), g) in enumerate(df.groupby(self.groupby_cols, sort=False)):
            ...
            def _soft_recip_pos(dv: np.ndarray, eps: float, interior: np.ndarray) -> np.ndarray:
                out = np.full_like(dv, self.padding_value, dtype=np.float32)
                if interior.any():
                    out[interior] = (1.0 / np.maximum(np.abs(dv[interior]), eps)).astype(np.float32)
                return out

            if compute_dqdv:
                eps_ch = self.dqdv_eps_mult * _robust_location_scale(dVdQ_ch_g[dqdv_interior_ch])[1] + 1e-12
                eps_dis = self.dqdv_eps_mult * _robust_location_scale(dVdQ_dis_g[dqdv_interior_dis])[1] + 1e-12
                dQdV_ch_g = _soft_recip_pos(dVdQ_ch_g, eps_ch, dqdv_interior_ch)
                dQdV_dis_g = _soft_recip_pos(dVdQ_dis_g, eps_dis, dqdv_interior_dis)
            else:
