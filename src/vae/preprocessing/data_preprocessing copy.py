...
class DataProcessor:
    ...
    def create_capacity_features_and_masks(
        ...
        for k, ((ds, cyc), g) in enumerate(df.groupby(self.groupby_cols, sort=False)):
            ...
            if compute_ica:
                eps_ch = self.ica_eps_mult * _robust_location_scale(dVdQ_ch_g[ica_interior_ch])[1] + 1e-12
                eps_dis = self.ica_eps_mult * _robust_location_scale(dVdQ_dis_g[ica_interior_dis])[1] + 1e-12
                dQdV_ch_g = _soft_recip_pos(dVdQ_ch_g, eps_ch, ica_interior_ch)
                dQdV_dis_g = _soft_recip_pos(dVdQ_dis_g, eps_dis, ica_interior_dis)
            else:
